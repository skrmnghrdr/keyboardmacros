...
enum custom_keycodes {
    MY_ALT = SAFE_RANGE,
    KC_PANIC, //panic key when things get stuck
};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [0] = LAYOUT_tkl_ansi( //defualt key layer 
        ...
        KC_LCTL, KC_LGUI,  LT(2, KC_LALT),                        KC_SPC,                                KC_RALT,  KC_APP,   KC_RCTL,  TG(3) // go to vanilla mode
    ),
    ...
    [3] = LAYOUT_tkl_ansi( //fn key layer (keyboard things)
        ...
        KC_LSFT, KC_Z,     KC_X,     KC_C,      KC_V,      KC_B,     KC_N,     KC_M,    KC_COMM,  KC_DOT,             KC_SLSH,            KC_RSFT,
        KC_LCTL, KC_LGUI,  KC_LALT,                        KC_SPC,                                KC_RALT,  KC_APP,   KC_RCTL,  TG(3) //go back to home macros
    ) //USE TO(x) if you want to go to a layer at this point
}; 
